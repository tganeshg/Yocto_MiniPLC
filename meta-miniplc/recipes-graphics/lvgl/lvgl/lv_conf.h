 ...
 *  32-bit required for LVGL Linux DRM backend on Pi (KMS); fbdev still works. */
...
#define LV_USE_LINUX_FBDEV      1
    ...
    /* Direct: one full-size draw buffer. More RAM, but avoids partial-flush issues
     * on some ARM framebuffers (KMS/simplefb) that left a black screen. */
...
/** Driver for /dev/dri/card — preferred on Raspberry Pi KMS (fixes black panel with fbdev-only). */
